...
%The \texttt{creatGame} function allows only operator, enusred by \texttt{byOperator} modifier, to creat a game  given that the contract is in \texttt{IDLE} state, which is eforced by inState(s) modifier.
%
%The \texttt{creatGame} function assigns a value to \texttt{hasNumber} and changes the state of the contract to \texttt{GMAE\_AVAILABLE}. 
%
%The \texttt{hasNumber} value provided by the operator when creating game is later used to ensure the bet was placed by the operator in the begining of game.
%
%
%


%A player may place a bet by calling \texttt{placeBet} function in \texttt{GMAE\_AVAILABLE} state. 
%
%The \texttt{placeBet} function accepts a guess, \texttt{\_guess}, and changes the sate to \texttt{BET\_PLACED}. 
%
%Moreovoer, \texttt{require} is used to exclude operator from the bet and also place maximum limit on the bet money.
%

%In \texttt{BET\_PLACED} state, only operator has liberty to add money by invoking \texttt{addToPor} in the pot (due to \texttt{byOperator} modifier). 
%
%However, operator cannot remove money from the pot by calling \texttt{removeFromPot} function due \texttt{noActiveBet} modifier.
%
%It ensures the fairness of the betting process.
%
